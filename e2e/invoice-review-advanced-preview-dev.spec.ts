...
import {
  fetchReviewInvoice,
  openAdvancedReviewPreview,
} from "./helpers/invoice-advanced-preview-flow";
...
test.describe("Invoice review — advanced overlay preview (dev)", () => {
  test.beforeAll(async ({}, testInfo) => {
    testInfo.skip(!hasDevE2eEnv(), missingEnvMessage);
  });

  test("advanced review UI only (skip upload when env set)", async ({ page, baseURL }) => {
    const keys = devReviewOnlyKeys();
    test.skip(!keys, "Set PLAYWRIGHT_REVIEW_INVOICE_ID and PLAYWRIGHT_REVIEW_SESSION_ID");

    await openAdvancedReviewPreview({
      page,
      baseURL,
  ...
  test("sample PNG upload → advanced review SPA (image + field overlays)", async ({ page, baseURL }) => {
    ...
    console.log(
      "[e2e] Full run: Cognito upload → Textract → AWAITING_HUMAN (often 1–3 min on dev). " +
        "For a fast re-run after one success, set PLAYWRIGHT_REVIEW_INVOICE_ID + PLAYWRIGHT_REVIEW_SESSION_ID.",
    );
    ...
    await test.step("Open advanced review SPA", async () => {
